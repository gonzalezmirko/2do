...
public class JobSchedulerTest {
    ...
    //Tuve que modifical todos los new JobScheduler para que me funcione el test= aca en
    //el beforeEach
    void setUp() {
        this.initializeJobs();
    }
    ...
    private JobScheduler newFifoScheduler() {
        JobScheduler fifoScheduler = new JobScheduler(new FifoStrategy());
        //fifoScheduler.setStrategy("FIFO");
        return fifoScheduler;
    }

    private JobScheduler newLifoScheduler() {
        JobScheduler lifoScheduler = new JobScheduler(new LifoStrategy());
        //lifoScheduler.setStrategy("LIFO");
        return lifoScheduler;
    }

    private JobScheduler newPriorityScheduler() {
        JobScheduler priorityScheduler = new JobScheduler(new HighestPriorityStrategy());
        //priorityScheduler.setStrategy("HighestPriority");
        return priorityScheduler;
    }

    private JobScheduler newEffortScheduler() {
        JobScheduler effortScheduler = new JobScheduler(new MostEffortStrategy());
    ...
    @Test
    //Pero aca tambien en los Test dentro del constructor que puse cada Job
    //y paso los test pero no se si esta bien
    void testSchedule() {
        JobScheduler aScheduler = new JobScheduler(new HighestPriorityStrategy());
